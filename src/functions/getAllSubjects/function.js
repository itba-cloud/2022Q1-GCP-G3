...
exports.getAllSubjects = async (req, res) => {
  ...
  pool.connect((error, client, release) => {
    ...
    client.query(`SELECT s.*, CASE WHEN su.user_id = ${id} THEN 1 ELSE 0 END AS isEnrolled FROM subjects s LEFT JOIN subjects_users su ON s.id = su.subject_id ORDER BY s.id`)
    .then(res => {
      code = 200;
      response = res.rows;
    })
